...
export function* getOrderBeingTransportedStatus() {
  yield takeLatest(OrderActions.ORDER_BEING_TRANSPORTED_STATUS_REQUEST, doGetOrderBeingTransportedStatus);
}
...
function* doImportWaybillCode(data) {
  ...
  try {
    ...
    if (response.data.code === api.HTTP_ERROR) {
      yield put({type: OrderActions.IMPORT_WAYBILL_CODE_FAIL, data: response.data})
    }
  }
  catch(error) {
    yield put({type: SystemActions.SYSTEM_ERROR})
