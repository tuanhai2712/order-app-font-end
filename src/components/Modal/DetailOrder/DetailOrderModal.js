...
function DetailOrderModal(props) {
  ...
  const renderPriceForMass = (mass) => {
    ...
    if (mass) {
      if (0 < mass <= 20) {
        if (chuyen_nhanh) {
          return mass * freight_charges_ls_slow_20;
        }
        return mass * freight_charges_ls_fast_20;
      } else if (20 < mass <= 100) {
        if (chuyen_nhanh) {
          return mass * freight_charges_ls_slow_20_100;
        }
        return mass * freight_charges_ls_fast_20_100;
      }
      if (chuyen_nhanh && store === 2) {
        return mass * freight_charges_hn_slow_100;
      }
      if (!chuyen_nhanh && store === 2) {
        return mass * freight_charges_hn_fast_100;
      }
      if (chuyen_nhanh && store === 3) {
        return mass * freight_charges_hcm_slow_100;
      }
      if (!chuyen_nhanh && store === 3) {
        return mass * freight_charges_hcm_fast_100;
      }
      if (chuyen_nhanh) {
        return mass & freight_charges_ls_fast_100;
      }
      return mass * freight_charges_ls_slow_100;
    }
  };
              ...
              <ModalBody>
                    ...
                    <Form className="form-container">
                      ...
                      <FormGroup className="check-fast">
                        <Input
                          type="radio"
                          name="radio1"
                          checked={!orderDetail.chuyen_nhanh}
                          disabled
                        />{' '}
                        Chuyển nhanh
                      </FormGroup>
                    ...
                    <Form className="form-container">
                      ...
                      <FormGroup className="group-field">
                        ...
                        <Input
                          ...
                          value={
                            Math.round(
                              parseInt(orderDetail.gia_thuc_te) * 0.03 * 1000
                            ) / 1000
                          }
                        />
                      </FormGroup>
                ...
                <Form className="form-total-container">
                  <FormGroup className="form-total">
                    <Label className="title-form-total">
                      Tổng phí [1 x (2 + 3 + 4) + 5 + 6]
                    </Label>
                    <Label className="text-form-total">{`${String(
                      parseInt(total())
                    ).replace(regex.price, '$1.')}  VNĐ`}</Label>
                  </FormGroup>
                  <FormGroup className="form-total">
                    <Label className="title-form-total">
                      Đã thanh toán [7]
                    </Label>
                    <Label className="text-form-total">
                      {String(parseInt(orderDetail.dat_coc) || 0).replace(
                        regex.price,
                        '$1.'
                      )}{' '}
                      VNĐ
                  ...
                  <FormGroup className="form-total">
                    ...
                    <Label className="text-form-total">
                      {String(
                        parseInt(total()) - parseInt(orderDetail.dat_coc)
                      ).replace(regex.price, '$1.') || 0}{' '}
                      VNĐ
                    </Label>
                  </FormGroup>
                </Form>
