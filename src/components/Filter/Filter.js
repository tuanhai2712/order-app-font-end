...
export default function Filter() {
  ...
  const [conditions, setConditions] = React.useState(useSelector(state => state.filterConditions))
  const [err, setErr] = React.useState({})
  const user = JSON.parse(localStorage.getItem("user"))
  const handleChangeTime = (value, name) => {
    setConditions({...conditions, [name]: moment(value).format('DD-MM-YYYY')})
  }
          ...
          <Grid container>
            <Grid xs={12} lg={3} sm={6} md={6} item>
              <div className="filter-container">
                <FormGroup>
                  ...
                  />
                  {err && err.tu_ngay && <p className="err-mess">{err.tu_ngay}</p>}
                </FormGroup>
                <FormGroup>
                  <span>Đến ngày</span>
                  <Datetime
                  ...
                  />
                  {err && err.den_ngay && <p className="err-mess">{err.den_ngay}</p>}
                </FormGroup>
              </div>
            </Grid>
            <Grid xs={12} lg={3} sm={6} md={6} item>
            ...
            <Grid xs={12} lg={3} sm={6} md={6} item>
              <FormGroup className="form-group-filter">
                ...
                <Input type="select" name="tinh_trang" id="exampleSelect" className="input-select-filter" onChange={e => handleChange(e)}>
                  <option value={8}>Tất cả</option>
                  {statistic.map((i,k) => {
                    return <option key={k} value={k}>{i}</option>
                  })}
                </Input>
              </FormGroup>
            </Grid>
