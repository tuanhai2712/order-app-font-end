...
export default function ImportListConsignment() {
  ...
  const importWaybillCodeResult = useSelector(
    (state) => state.importWaybillCodeResult
  );
    ...
    <GridContainer>
      <GridItem xs={12} sm={12} md={12}>
        <div className="import-container">
          {importResult.code === api.HTTP_OK ? (
            <Success
              mess={'Import mã vận đơn thành công'}
              visible={importResult.code === api.HTTP_OK}
            />
          ) : null}
          {importResult.code === api.HTTP_ERROR ? (
            <Error
              mess={importResult.error}
              visible={importResult.code === api.HTTP_ERROR}
            />
          ) : null}
          {errorFileType ? (
            <Error mess={'File không đúng định dạng'} visible={errorFileType} />
          ) : null}
          <Form>
            ...
            {loading && <Loading />}
            <FileUpload getFile={(file) => getFile(file)} />
          </Form>
