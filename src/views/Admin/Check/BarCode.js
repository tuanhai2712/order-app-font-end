...
export default function BarCode() {
  ...
  const transfer = () => {
    if (!isEmpty(order)) {
      ...
      if (isEmpty(error)) {
        const findOrderConfirmed = confirmedOrder.find((i, k) => (i.ma_van_don === order.ma_van_don))
        if (isEmpty(findOrderConfirmed)) {
          ...
          confirmedOrder.map((i, k) => {
            if (isEmpty(waybillCode)) {
              setWaybillCode(`${i.ma_van_don},`)
            } else {
              setWaybillCode(`${waybillCode} ${i.ma_van_don},`)
            }
          })
        }
      }
  ...
  const handleScan = (code) => {
    ...
    setOrder({})
    setCustomer("")
    const findOrderWithWaybillCode = orders.find((i, k) => (i.ma_van_don === code))
    const findConsignmentWithWaybillCode = consignment.find((i, k) => (i.ma_van_don === code))
    if (!isEmpty(findOrderWithWaybillCode)) {
      const findCustomer = customers.find((i, k) => (i.id === parseInt(findOrderWithWaybillCode.user_id)))
      setOrder(findOrderWithWaybillCode)
      setCustomer(`${findCustomer.name} - ${findCustomer.address} - ${findCustomer.phone_number}`)
    }
    if (!isEmpty(findConsignmentWithWaybillCode)) {
      let arr = arrConsignment
      if (!arr.find(element => element === findConsignmentWithWaybillCode.ma_van_don)) {
        arr.push(findConsignmentWithWaybillCode.ma_van_don)
        setArrConsignment(arr)
        if (!waybillCodeConsignment.length) {
          setWaybillCodeConsignment(`${findConsignmentWithWaybillCode.ma_van_don},`)
        } else {
          setWaybillCodeConsignment(`${waybillCodeConsignment} ${findConsignmentWithWaybillCode.ma_van_don},`)
        }
      }
    }
    if (isEmpty(findConsignmentWithWaybillCode) && isEmpty(findOrderWithWaybillCode)) {
      setErr({...err, ["not_found"]: `Không tìm thấy đơn hàng với mã ${code}`})
    }
  }
    ...
    <GridContainer>
      <GridItem xs={12} sm={12} md={12}>
        ...
        <div className="check-container">
          {checkBarcodeResult && <Success mess={"Check đơn thành công"} visible={checkBarcodeResult}/>}
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="content">
                          <Input type="number" name="khoi_luong" placeholder="Nhập khối lượng sản phẩm" value={order.khoi_luong} onChange={e => handleChange(e)}/>
                        </td>
                      </tr>
                      <tr>
