...
function HomePage() {
  ...
  const changeSettingLoading = useSelector(
    (state) => state.changeSettingLoading
  );
  const settings = useSelector((state) => state.settings);
  const imgProcudure =
    settings && settings.procedure_img
      ? `${api.BASE_URL}${settings.procedure_img}`
      : require('assets/img/defaut.png');
  const imgPrice =
    settings && settings.price_img
      ? `${api.BASE_URL}${settings.price_img}`
      : require('assets/img/defaut.png');
  const imgPay =
    settings && settings.pay_img
      ? `${api.BASE_URL}${settings.pay_img}`
      : require('assets/img/defaut.png');
                ...
                <TabPane tabId="pills1">
                  <Col className="ml-auto mr-auto" md="10">
                    <Row className="collections">
                      ...
                      {changeSettingLoading.status &&
                        changeSettingLoading.field === 'procedure_img' && (
                          <Loading />
                        )}
                ...
                <TabPane tabId="pills2">
                  <Col className="ml-auto mr-auto" md="10">
                    <Row className="collections">
                      ...
                      {changeSettingLoading.status &&
                        changeSettingLoading.field === 'price_img' && (
                          <Loading />
                        )}
                ...
                <TabPane tabId="pills3">
                  <Col className="ml-auto mr-auto" md="10">
                    <Row className="collections">
                      ...
                      {changeSettingLoading.status &&
                        changeSettingLoading.field === 'pay_img' && <Loading />}
